...
def create_sequences(data, target, sequence_length=30):  # Increased sequence length for better learning
    X, y = [], []
...
def build_model(hp):
    ...
    model.add(Bidirectional(LSTM(units=hp.Int('units2', min_value=64, max_value=256, step=64), return_sequences=True)))
    model.add(Dropout(hp.Float('dropout2', 0.1, 0.5, step=0.1)))

    model.add(Bidirectional(LSTM(units=hp.Int('units3', min_value=32, max_value=128, step=32))))
    model.add(Dropout(hp.Float('dropout3', 0.1, 0.5, step=0.1)))

    model.add(Dense(units=1))  # Output layer

    model.compile(optimizer=RMSprop(hp.Choice('learning_rate', [0.001, 0.0005, 0.0001])),
...
tuner = RandomSearch(
    build_model,
    objective='val_loss',
    max_trials=10,  # Increased trials for better optimization
    executions_per_trial=3,  # Runs each trial thrice for stability
    directory='lstm_tuner',
    project_name='wind_speed_prediction'
)

# Search for the best hyperparameters
tuner.search(X_train, y_train, epochs=20, validation_data=(X_test, y_test), batch_size=32)  # Optimized batch size
...
threshold = np.percentile(y_test_rescaled, 50)  # Dynamic threshold
